...
class DataProcessor:
    ...
    def _get_range_start(self) -> str:
        ...
        """
        # if table exit and it's not processed result will be null,
        ...
        if start is None:
            logger.info(
                "No existing arxiv_papers table or no processed records in the table"
            )
            start = time.strftime("%Y%m%d%H%M", time.gmtime(time.time() - 24 * 3600 * 3))
            logger.info(f"Starting from 3 days ago: {start}")
        # make sure start is string bcz its used in string formatting for arxiv search query
        return str(start)
    ...
    def download_and_store_papers(
        ...
        client = arxiv.Client()
        search = arxiv.Search(
            query=f"cat:cs.AI AND submittedDate:[{start} TO {self.end}]", max_results=100
        )
        logger.info(f"Searching arXiv for papers with query: {search.query}")
